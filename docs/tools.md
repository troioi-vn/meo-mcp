...
| `upload_ledger_transaction_receipt_from_url` | Live | Add or replace one exact transaction receipt from a bounded public HTTPS image/PDF | `finance:read` + `finance:write` | `finance:read` + `finance:write` (legacy PAT: `read` + `update`) | transaction preview; guarded source `GET`; multipart `POST .../transactions/{transaction_id}/receipt`; detail verification | Update | Critical; stores/replaces a private financial document |
| `delete_ledger_transaction_receipt` | Live | Delete the previewed receipt from one exact versioned transaction | `finance:read` + `finance:write` | `finance:read` + `finance:write` (legacy PAT: `read` + `delete`) | transaction preview; `DELETE .../transactions/{transaction_id}/receipt`; detail verification | Delete | Critical; permanently removes a private financial document |
...
| `list_pet_categories` | Live | Resolve visible breed/category options for one exact species | `pets:read` | `pets:read` (legacy PAT: `read`) | pet-type lookup; `GET /api/categories` | Read | Low; reference data plus caller-created pending options |
| `create_pet_category` | Live | Create one caller-visible category option for an exact species after duplicate preview | `pets:read` + `pets:write` | `pets:read` + `pet:write` (legacy PAT: `read` + `create`) | category preview; `POST /api/categories`; category verification | Create | Moderate; creates moderated shared reference data |
| `update_pet_status` | Live | Change one exact pet between active, lost, and deceased states from a versioned preview | `pets:read` + `pets:write` | `pets:read` + `pet:write` (legacy PAT: `read` + `update`) | pet preview; `PUT /api/pets/{pet_id}/status`; pet verification | Update | High; changes visibility and lifecycle presentation |
...
| `duplicate_candidate` | no | A create matched an existing pet, pet category, helper city, group, ledger, owner-weight date, or pending invitation target; inspect stable IDs before deciding whether this is distinct intent |
