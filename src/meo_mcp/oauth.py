...
class DatabaseOAuthProvider(OAuthAuthorizationServerProvider[AuthorizationCode, RefreshToken, AccessToken]):
    ...
    async def register_client(self, client_info: OAuthClientInformationFull) -> None:
        if not client_info.client_id or not client_info.redirect_uris:
            raise TokenError("invalid_client", "A public client ID and redirect URI are required.")
        async with self.sessions() as session:
            present = await session.scalar(select(OAuthClient).where(OAuthClient.client_id == client_info.client_id))
            if present:
                return
            session.add(OAuthClient(client_id=client_info.client_id, client_metadata=client_info.model_dump(mode="json")))
            await session.commit()

    async def authorize(self, client: OAuthClientInformationFull, params: AuthorizationParams) -> str:
        if params.resource != self.settings.resource:
            raise AuthorizeError("invalid_request", "The resource parameter must name this MCP endpoint.")
    ...
    async def _exchange_meo_code(self, code: str) -> dict:
        ...
        async with httpx.AsyncClient(timeout=10) as client:
            response = await client.post(f"{str(self.settings.meo_base_url).rstrip('/')}/api/mcp-auth/exchange", json={"code": code}, headers=headers)
    ...
    async def exchange_refresh_token(self, client: OAuthClientInformationFull, refresh_token: RefreshToken, scopes: list[str]) -> OAuthToken:
        async with self.sessions() as session:
            ...
            if record.consumed_at:
                ...
                await session.execute(update(Grant).where(Grant.id == record.grant_id).values(revoked_at=now()))
                await session.commit()
                raise TokenError("invalid_grant", "Refresh token replay detected; token family revoked.")
    ...
    async def _issue_tokens(self, session: AsyncSession, grant: Grant, client_id: str, scopes: list[str], family_id=None) -> OAuthToken:
        access_value, refresh_value = token(), token()
        expiry = now() + ACCESS_TOKEN_TTL
        refresh_expiry = min(now() + REFRESH_TOKEN_TTL, grant.expires_at)
        session.add_all([
            AccessTokenRecord(token_hash=digest(access_value), grant_id=grant.id, client_id=client_id, scopes=scopes, subject=grant.subject, resource=self.settings.resource, expires_at=expiry),
            RefreshTokenRecord(token_hash=digest(refresh_value), family_id=family_id or uuid4(), grant_id=grant.id, client_id=client_id, scopes=scopes, subject=grant.subject, expires_at=refresh_expiry),
        ])
        return OAuthToken(access_token=access_value, expires_in=int(ACCESS_TOKEN_TTL.total_seconds()), scope=" ".join(scopes), refresh_token=refresh_value)
    ...
    async def load_access_token(self, value: str) -> AccessToken | None:
        async with self.sessions() as session:
            ...
            if not grant or grant.revoked_at:
                return None
            return AccessToken(token=value, client_id=record.client_id, scopes=record.scopes, expires_at=record.expires_at.timestamp(), resource=record.resource, subject=record.subject)

    async def revoke_token(self, value: AccessToken | RefreshToken) -> None:
        async with self.sessions() as session:
            access = await session.scalar(select(AccessTokenRecord).where(AccessTokenRecord.token_hash == digest(value.token)))
            refresh = await session.scalar(select(RefreshTokenRecord).where(RefreshTokenRecord.token_hash == digest(value.token)))
            grant_id = access.grant_id if access else refresh.grant_id if refresh else None
            if grant_id:
                await session.execute(update(Grant).where(Grant.id == grant_id).values(revoked_at=now()))
                await session.execute(update(AccessTokenRecord).where(AccessTokenRecord.grant_id == grant_id).values(revoked_at=now()))
                await session.execute(update(RefreshTokenRecord).where(RefreshTokenRecord.grant_id == grant_id).values(revoked_at=now()))
                await session.commit()
