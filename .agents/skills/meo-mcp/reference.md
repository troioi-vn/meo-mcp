...
| Safe self-profile writes: display name/avatar/owner weights | exact current profile or weight version, expected target, idempotency key; public HTTPS source for avatar | verified narrowed profile/weight state or absence | `profile:read profile:write` | matching abilities | self-profile/avatar/owner-weight endpoints | create/update/delete | Critical |
