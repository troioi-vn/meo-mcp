...
| Vaccination: `list_vaccinations`, `get_vaccination` | explicit pet/record IDs; prefer `status=overdue` for past-due renewals and trust `is_overdue` | narrowed vaccination records | `health:read` | `health:read` | vaccination list/detail | read-only | Moderate |
...
| Health writes: add/update/delete weight, vaccination, medical record; vaccination renewal; guarded medical-media import/delete | explicit pet/record/photo IDs, exact expected target for destructive actions, idempotency key, current parent/record version, public HTTPS source for import | verified narrowed record/media or absence | `health:read health:write` | matching abilities | health and medical-media mutations plus verification reads | create/update/delete | Moderate to critical |
...
| Finance reads: currencies/ledgers/overview/transactions/receipts | explicit ledger/transaction/pet IDs plus bounded filters | narrowed configuration, totals, members, pets, transaction records, or bounded private receipt content | `finance:read` | `finance:read` | finance read and authenticated receipt endpoints | read-only | Critical |
| Finance writes: lifecycle/members/pets/group-link/config/transactions/receipts/invitations | exact stable targets, idempotency key, current ledger/account/category/transaction or invitation version; expected receipt state and guarded public HTTPS source for receipt import | verified ledger, configuration, transaction, receipt presence, or invitation state | `finance:read finance:write` | matching abilities | type-specific finance mutation and verification endpoints | create/update/delete | Critical |
